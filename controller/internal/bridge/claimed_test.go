...
func TestClaimed_HandleClosed_ClaimedTask_NudgesCheckpoint(t *testing.T) {
	...
	if msg == "" {
		t.Fatal("expected checkpoint nudge for claimed bead closure, got none")
	}
	want := `Your claimed bead kd-xyz "Implement the feature" was closed — create a decision checkpoint`
	if msg != want {
		t.Errorf("unexpected nudge message:\n  got:  %s\n  want: %s", msg, want)
	}
}
